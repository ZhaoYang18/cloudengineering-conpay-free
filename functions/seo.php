...
function add_category_seo_format(){  
   ...
   <div class="form-field">  
            <select name="catce" id="catce">
			 ...
			 <label for="navm">显示文章多重筛选【付费版解锁】</label>
   ...
   <div class="cat_modle2">  
            <select disabled >
			 ...
			 <option >默认图文结构（左图右文）</option>
             <option >纯文字结构【付费版解锁】</option>
			 <option >图片模式（可弹出播放视频）【付费版解锁】</option>
			 </select>
   ...
   <div class="cat_modle3">  
            <select disabled>
			 ...
			 <option >一行显示4个产品（默认）</option>
			  <option>一行显示3个</option>
			 <option >一行显示5个</option>
...
function edit_category_seo_format($tag){  
   ...
   <tr class="form-field">  
          <th scope="row"><label for="images">侧边栏选择【付费版解锁】</label></th>  
            <td> 
		 <select disabled >
	        <option>默认的侧边栏目</option>
	...
	<tr class="form-field">  
       ...
       <p>这个多重筛选是针对普通文章的，请和woocommerce的产品筛选区分开</p>
		...
		<tr class="form-field">  
          <th scope="row"><label for="cat_mr_jic">不继承默认设置</label></th>  
            <td> 
		...
		<tr class="form-field">  
          <th scope="row"><label for="images">模式选择【付费版解锁】</label></th>  
		   ...
		   <select disabled>
			 ...
			 <option   value="">两栏默认</option>
			 <option   value="tong">通栏【付费版解锁】</option>
	...
	<tr class="form-field">  
          <th scope="row"><label for="images">显示列数【付费版解锁】</label></th>  
            <td> 
		   <select disabled name="cat_modle3" id="cat_modle3">
	       
			
			 <option >一行显示3个产品（默认）</option>
			 <option >一行显示4个</option>
			 <option >一行显示5个</option>
			 <option >一行显示6个</option>
       ...
       <tr >  
          <th scope="row"><label for="cat_modle4">顶部默认图片</label></th>  
            <td> 
		  <input type="text" style="width:200px;" size="10" value="<?php echo get_option('cat_modle4_'.$tag->term_id) ?>" name="cat_modle4" id="cat_modle4"/>
         <a  class="left_right_upload_button button" href="#">上传</a>		
         <p>所有的分类顶部有一个背景图片，你可以在此替换，每一个分类你都可以设置不同的背景图片，这些设置在分类目录编辑中可以看到(图片的宽度为1920，高度可以是400，这个取决于你的分类描述占位高度)</p>
            </td>  
        </tr>   
...
function taxonomy_metadate_seo_format($term_id){  
    if(isset($_POST['cat_title']) || isset($_POST['cat_keword'])|| isset($_POST['cat_description'])||isset($_POST['cat_title_p']) || isset($_POST['cat_keword_p'])|| isset($_POST['cat_description_p'])|| isset($_POST['navm'])){  
		...
		$cat_modle4 ='cat_modle4_'.$term_id;
		$cat_modle4_value =$_POST['cat_modle4']; 
        ...
        $description_value = $_POST['cat_description']; 
		 ...
		 update_option( $cat_nummber, $cat_nummber_value );     
		  ...
		  update_option( $cat_modle4, $cat_modle4_value );  
