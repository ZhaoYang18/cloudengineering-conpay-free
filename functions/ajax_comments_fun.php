...
?>
	<li <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ) ?> id="comment-<?php comment_ID() ?>">
      ...
      <div id="ajax_commont_tex_li">
             <span class="top_ajax_span"><?php if(is_admin_comment($comment->comment_ID)){echo '<b class="admin_red">管理员'; }else{ echo  '<b>'.$author_name;} ?></b> 
             <?php if ( $comment->comment_approved == '0' ) {echo '<a class="admin_red">['.get_themepark_option('language_pl9','评论审核中').']</a>';}else{ ?>
             <?php if(!$thempark_commont){ ?><a class="hfpl" rel="<?php comment_ID() ?>"><?php echo get_themepark_option('language_pl0','回复') ?></a> <?php } ?>
			...
			if($themepark_comment_box_show=='showall'&&$thempark_commont){
             ...
             <br />
           <em><?php comment_date('Y-m-d') ?><?php comment_time() ?></em> 
