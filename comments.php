...
include("options/data_cache.php");
 ...
 if (is_user_logged_in()) { $post_url_go= get_option('siteurl').'/wp-comments-post.php'; $hellos=$language_pl1.'<a target="_blank" > '. $author_name.' </a>';
 
  }else{
	  if(!get_option('comment_registration')){  $post_url_go= get_option('siteurl').'/wp-comments-post.php';}else{ $ajax_no_login='<div class="ajax_no_login  bbs_no">'.$language_pl2 .'</div>';}
