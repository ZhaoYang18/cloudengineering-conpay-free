...
class nav_menu extends WP_Widget {
	...
	{
		$widget_ops = array('classname'=>'nav_menu','description' => get_bloginfo('template_url').'/images/xuanxiang/3.gif');
 ...
 <div class="widget woocommerce nav_menu" >       
   ...
   if(is_product_category()){
	 global $wp_query;
	    ...
	    $product_lkkb= get_themepark_option('product_lkkb','下属分类');
		$product_lkkb2= get_themepark_option('product_lkkb2','下属分类');
	   $this_top_cat='';
	 $cats=$wp_query->queried_object;
	$cat= get_category_root_id($cats->term_id);
	 $cat_n= get_category_root_id_n($cats->term_id);
	 $this_cat=  get_term($cats->term_id,'product_cat');
     $this_top_catid= $this_cat->parent;
	...
	if(get_terms("child_of=".$cat."&taxonomy=product_cat")){
	
   echo '<div class="widget woocommerce widget_nav_product"><div class="widget_title_t">'. $product_lkkb.'：</div><ul>';
   wp_list_categories("child_of=".$cat. "&depth=1&hide_empty=0&title_li=0&hierarchical=1&taxonomy=product_cat&show_option_none=");
   
  echo '</ul></div>';
		
		if($cat_n=='1'&&get_terms("child_of=".$cat."&taxonomy=product_cat&depth=1&hierarchical=1")&&get_terms("child_of=".$cats->term_id."&taxonomy=product_cat")&&$this_top_catid!=0){
		
		  echo '<div class="widget woocommerce widget_nav_product"><div class="widget_title_t">'. $product_lkkb2.'：</div><ul>';
         wp_list_categories("child_of=".$cats->term_id. "&depth=1&hide_empty=0&title_li=0&hierarchical=1&taxonomy=product_cat&show_option_none=");
   
       echo '</ul></div>';
		}
		
		if($cat_n=='2'&&!get_terms("child_of=".$cats->term_id."&taxonomy=product_cat")&&get_terms("child_of=".$this_top_catid."&taxonomy=product_cat&depth=2&hierarchical=1")&&$this_top_catid!=0){
		
		  echo '<div class="widget woocommerce widget_nav_product"><div class="widget_title_t">'. $product_lkkb2.'：</div><ul>';
         wp_list_categories("child_of=".$this_top_catid. "&depth=1&hide_empty=0&title_li=0&hierarchical=1&taxonomy=product_cat&show_option_none=");
