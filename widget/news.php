...
class news extends WP_Widget {
	...
	{
		$widget_ops = array('classname'=>'news','description' => get_bloginfo('template_url').'/images/xuanxiang/4.gif');
		$control_ops = array('width' => 200, 'height' => 300);
		parent::__construct($id_base="news",$name='新闻模块【首页】',$widget_ops,$control_ops);  
...
<p>
 ...
 <input type="text" size="40" readonly="readonly" value="1" />
...
<div id="news_index_show" <?php echo 'class="index_boxs '.$color.' "' ; echo $bac_colors;?> >
...
<a  class="list_nav_title" <?php if( $btn_link){echo ' href="'.$btn_link.'" target="_blank"';} ?> >
	...
	<div class="swiper-container news<?php echo ${'w_cat0'} ?>">
        ...
        <div class="swiper-wrapper"> 
          ...
          <div class="swiper-slide">
            <div class="news_box">
		        ...
		        $i=0;  while ( ${'query'.$x}->have_posts() ) :${'query'.$x}->the_post(); $i++;
		        if($i==1){news_loops('large',$titleseo3,$seo_out2,1);}

		        endwhile;  wp_reset_query();endif;	
		
				if(${'query'.$x}->have_posts()) : 
		        echo '<ul class="news_list_index">';
		        $i=0;  while ( ${'query'.$x}->have_posts() ) :${'query'.$x}->the_post(); $i++;
		        if($i!=1){news_loops('medium',$titleseo3,$seo_out2,2);}
