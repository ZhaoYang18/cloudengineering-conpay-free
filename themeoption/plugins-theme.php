<?php
/* 初始化设置*/
add_action( 'user_register', 'close_user_can_posts' );
function close_user_can_posts( $user_id ) {
$wps_uesr_can_post=get_option('wps_uesr_can_post');
$wps_uesr_can_post_t=get_option('wps_uesr_can_post_t');
    update_user_meta( $user_id, 'wpuf_postlock', 'yes');
    update_user_meta( $user_id, 'wpuf_lock_cause', $wps_uesr_can_post_t );
} 
...
function my_info_in_myaccount(){
	...
	if(get_option('mytheme_myaccount_gonggao')){$mytheme_myaccount_gonggao=stripslashes(get_option('mytheme_myaccount_gonggao'));}else{$mytheme_myaccount_gonggao='你好！欢迎登陆WEB主题公园。<br> 
你可以在这里写些什么内容投稿给我们，同时有什么问题也可以提交工单给我们。';};//个人中心公
...
<?php }
function plugin_theme_css() {
	$wp_user_frontend_pro_theme = get_option('wp_user_frontend_pro_theme');
	if ( !is_admin()&&$wp_user_frontend_pro_theme) {
		
		  
	   wp_deregister_style('plugin_theme');
	   wp_register_style( 'plugin_theme', get_template_directory_uri() .'/css/plugin_theme.css');
	   wp_enqueue_style('plugin_theme');
		
		}
	
	}
add_action('wp_print_styles', 'plugin_theme_css');
...
专业版下载：[<a target="_blank" href="http://pan.baidu.com/s/1c2lhYGK">百度网盘</a>],提取码：a6zh</p>
             ...
             <tr>
       <th scope="row"><label for="wps_uesr_can_post">开启会员申请发布文章审核<?php echo $wps_uesr_can_post; ?></label>
                </th>
                <td>
                         <input type="checkbox" <?php if($wps_uesr_can_post){echo 'checked="checked" ';}; ?>size="60"  name="wps_uesr_can_post" id="wps_uesr_can_post" value="yes"/>  <br />
